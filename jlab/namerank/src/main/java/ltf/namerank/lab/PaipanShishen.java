...
public class PaipanShishen {
    ...
    private String paipan(Calendar cal, PaipanLvhehun.sex isman) throws ParseException {
        ...
        for (int i = 0; i < DayunArray.length; i++) {
            DayunArrayshengsi[i] = shengwang[0][myPaipanDayun.getyuezhuStart(shengwang[shengsibiaoshunxu], DayunArray[i].substring(1, 2))]; //十天干生旺死绝表 用干查表  

            DayunArrayshisheng[i] = shisheng[shishengbiaoshunxu][myPaipanDayun.getyuezhuStart(shisheng[0], DayunArray[i].substring(0, 1))]; //十神表 用支查表  
        }
        System.out.println("此人大运");
        myPaipanDayun.pringst(DayunArray);
        System.out.println("此人大运生旺死绝");
        myPaipanDayun.pringst(DayunArrayshengsi);
        System.out.println("此人大运十神");
        myPaipanDayun.pringst(DayunArrayshisheng);
        return null;
    }
